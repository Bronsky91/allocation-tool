export const createAllocationAccountString = (data, allocationChartField) => {
    ...
    const otherSegments = data.segments
        ...
        .map(s => {
            if (s.description === 'GL Code') {
                ...
                return {
                    chartField: data.otherSegments.find(os => os._id === s._id).selectedSubSegment.segmentId,
                    chartFieldOrder: s.chartFieldOrder
                }
            }
        })
    ...
    const combinedSegments = [...otherSegments, allocationChartFieldWithOrder]
...
export const createBalanceAccountString = (data) => {
    const segments = data.segments
        .map(s => {
            if (s.description === 'Sub-GL Code') {
                return {
                    chartField: data.subGLSegment.balance.segmentId,
                    chartFieldOrder: s.chartFieldOrder
                }
            } else if (data.selectedBalanceSegments.map(bs => bs._id).includes(s._id)) {
                return {
                    chartField: data.selectedBalanceSegments.find(bs => bs._id === s._id).selectedSubSegment.segmentId,
                    chartFieldOrder: s.chartFieldOrder
                }
            }
        }).sort((a, b) => a.chartFieldOrder - b.chartFieldOrder)
